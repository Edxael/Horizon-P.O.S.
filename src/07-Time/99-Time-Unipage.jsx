...
const MUTATION = gql`
  ...
  {
    updateTimeRecord( id: $id, active: $active, category: $category, name: $name, price: $price, skbc: $skbc, stock: $stock )
    {
      id
      active
      category
...
class UpdateTimeRecord extends Component {
  constructor(props){
    super(props)
  ...
  componentDidMount(){
    let TimeRecordsDb = MyLocStorage.get('lm_allTimeRecords')
    const token = this.props.match.params.id
    let currentTimeRecord = TimeRecordsDb.filter((x)=>{ return x.id === token }).reduce((z)=>{return z})

    this.setState({
      in: currentTimeRecord.in ,
      out: currentTimeRecord.out ,
      user: currentTimeRecord.user.id ,
      id: currentTimeRecord.id
    })
          ...
          { this.state.show ?
            ...
            <div>
              ...
              <div style={Infocont}>
                ...
                <form>
                    <div style={UInfo}> <label style={labelSty}>TimeRecord Name:  </label> <input style={inputSty} type="text" value={this.state.name} onChange={ (event) => { this.setState({ name: event.target.value }) } } /></div>
                ...
                <br/>
                <div>
                  <div><strong>TimeRecord ID: </strong>{this.state.id}</div>
                </div>
            ...
            : <div>Loading TimeRecord Data....</div>
          }

          <button style={btnSty} onClick={()=>{this.exe1()}} >Update TimeRecord</button>
...
export default graphql(MUTATION)(UpdateTimeRecord)
